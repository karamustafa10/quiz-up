...
function LiveQuizStudentPage() {
  ...
  const handleAnswer = (option) => {
    if (!selected && questionData) {
      setSelected(option);
      socket.emit('submit-answer', { sessionId, answer: option.toUpperCase() });
    }
  };
  ...
  const getRank = () => {
    ...
    const username = (user?.username || 'Bilinmeyen Öğrenci').trim().toLowerCase();
  
    const index = sorted.findIndex(entry =>
      (entry.name || '').trim().toLowerCase() === username
    );
            ...
            <div className="w-full flex flex-col gap-4">
              ...
              <div className="w-full flex flex-col gap-2 mb-2">
                {Array.isArray(scoreboard) && scoreboard.sort((a, b) => b.score - a.score).slice(0, 5).map((entry, i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-center justify-between gap-3 px-4 py-2 rounded-lg border border-neutral/20 dark:border-neutral-dark/30 bg-neutral/10 dark:bg-base-dark/40 ${isMe ? 'font-bold text-primary bg-primary/10 dark:bg-primary-dark/30 border-primary/40 dark:border-primary-dark/40' : 'text-neutral-dark dark:text-neutral'}`}>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-xl">🧑‍🎓</span>
                <span className="italic text-primary-dark dark:text-primary-light">Quiz'i {getRank()}. sırada tamamladınız.</span>
              </div>
            ...
            {questionData && (
              <>
                ...
                <ul className="list-none p-0 w-full flex flex-col gap-2 mb-4">
                  {['A', 'B', 'C', 'D'].map((opt) => {
                    let style = `py-2 px-4 rounded-xl shadow-sm text-base cursor-pointer bg-neutral text-neutral-dark hover:bg-primary/10 dark:bg-neutral-dark dark:text-white dark:hover:bg-primary-dark/30`;
                    if (correctOption) {
                      if (opt === correctOption && selected === opt) style = `py-2 px-4 rounded-xl shadow-sm text-base bg-success/80 text-white dark:bg-success-dark dark:text-white`;
                      else if (opt === selected && opt !== correctOption) style = `py-2 px-4 rounded-xl shadow-sm text-base bg-danger/80 text-white dark:bg-danger-dark dark:text-white`;
                      else if (opt === correctOption) style = `py-2 px-4 rounded-xl shadow-sm text-base bg-success/80 text-white dark:bg-success-dark dark:text-white`;
                    } else if (selected === opt) {
                      style = `py-2 px-4 rounded-xl shadow-sm text-base bg-primary/80 text-white dark:bg-primary-dark dark:text-white`;
                    }
                    if (questionLocked) style += ' opacity-60 pointer-events-none';
                    return (
                      <li key={opt} className={style} onClick={() => !questionLocked && handleAnswer(opt.toUpperCase())}>
              ...
              <div className="w-full mb-4 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-2 bg-success/10 dark:bg-success-dark/20 border border-success/40 dark:border-success-dark/40 rounded-xl px-4 py-2">
                  ...
                  {selected && selected === correctOption && (
                    <span className="ml-4 animate-bounce text-success text-base">🎉 Tebrikler, doğru bildiniz!</span>
                  )}
                  {selected && selected !== correctOption && (
                    <span className="ml-4 text-danger text-base">❌ Yanlış cevap verdiniz.</span>
                  )}
                </div>
                {/* Cevap Dağılımı */}
...
export default LiveQuizStudentPage;
